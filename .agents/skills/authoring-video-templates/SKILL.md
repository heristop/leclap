...
- **Looks & grade** — `look` is a preset: `eq`/`curves`-based (`cinematic`/`warm`/`cool`/`vintage`/`noir`/`vivid`/`dreamy`) or **LUT-backed** (`teal-orange`/`warm-film`/`mono-film`/`noir-film`/`vivid-pop` → a single `lut3d` + a bundled `.cube`, a stronger grade; staged like fonts, runs on every backend, dropped-with-warning where `lut3d` is absent). `grade` (brightness/contrast/saturation/gamma/hue/colorBalance/blur/curvesPreset) → `eq`/`colorbalance`/`curves`/`gblur`/`hue`, stacks on top.
