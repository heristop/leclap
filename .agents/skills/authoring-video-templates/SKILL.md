...
- **Motion** — ordered `motion[]`: `kenburns` (image_background only), `rotate`, `crop`, `flip`, `shake` (handheld jitter via a wandering `crop` window, `intensity`/`frequency`), `pulse` (rhythmic zoom via `zoompan`, `intensity`/`frequency`) → `zoompan`/`rotate`/`crop`/`hflip`/`vflip`. `shake`/`pulse` have no section-type restriction.
