...
function getPreviewDimensions(orientation: 'portrait' | 'landscape', fullscreen: boolean) {
  ...
  if (fullscreen) { return { width: windowWidth, height: windowHeight }; }
...
interface TimerOverlayProps { isPortrait: boolean; recordingDuration: number; }
...
interface FlipButtonProps { isPortrait: boolean; isRecording: boolean; onPress: () => void; }
...
interface DescriptionOverlayProps { isPortrait: boolean; description: string; onDismiss: () => void; }
function DescriptionOverlay({ isPortrait, description, onDismiss }: DescriptionOverlayProps) {
  return (
    <TouchableOpacity
      style={[styles.descriptionOverlay, isPortrait ? styles.portraitDescriptionOverlay : styles.landscapeDescriptionOverlay]}
      onPress={onDismiss}
    >
...
function useCameraPermissions() {
  ...
  useEffect(() => {
    ...
    checkPermissions().catch((error: unknown) => { console.error('checkPermissions failed:', error); });
  }, []);
...
function useRecordingActions({ cameraRef, setIsRecording, onVideoRecorded, orientation, device }: RecordingActionsParams) {
  const startRecording = async () => {
    ...
    try {
      ...
      cameraRef.current.startRecording({
        ...
        onRecordingFinished: (video) => { onVideoRecorded(video, orientation); setIsRecording(false); },
        onRecordingError: (error) => { console.error('Recording error:', error); setIsRecording(false); },
      });
    } catch (error) {
  ...
  const handleRecordPress = (isRecording: boolean) => {
    if (isRecording) {
      stopRecording().catch((error: unknown) => { console.error('stopRecording failed:', error); });

      return;
    }
    startRecording().catch((error: unknown) => { console.error('startRecording failed:', error); });
  };

  return { handleRecordPress };
}

const VideoRecorder: React.FC<VideoRecorderProps> = ({ orientation, onVideoRecorded, sectionDescription, fullscreen = false }) => {
  ...
  const { handleRecordPress } = useRecordingActions({ cameraRef, setIsRecording, onVideoRecorded, orientation, device });
  ...
  return (
    <View style={fullscreen ? styles.fullscreenContainer : styles.container}>
      ...
      <View style={[styles.controls, isPortrait ? styles.portraitControls : styles.landscapeControls]}>
        <TouchableOpacity style={[styles.recordButton, isRecording && styles.stopButton]} onPress={() => { handleRecordPress(isRecording); }}>
      ...
      {sectionDescription && showDescription && (
        <DescriptionOverlay isPortrait={isPortrait} description={sectionDescription} onDismiss={() => { setShowDescription(false); }} />
      )}
      <Text style={[styles.instructionText, isPortrait ? styles.portraitInstructions : styles.landscapeInstructions]}>
        {orientation === 'portrait' ? 'Hold your device vertically for best results' : 'Hold your device horizontally for best results'}
      </Text>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  timer: {
    ...
    paddingVertical: spacing.xs, borderRadius: 16, zIndex: 50,
  },
  ...
  flipCameraButton: {
    position: 'absolute', width: 50, height: 50, borderRadius: 25, backgroundColor: 'rgba(0, 0, 0, 0.5)',
  ...
  descriptionOverlay: {
    position: 'absolute', backgroundColor: 'rgba(0, 0, 0, 0.7)', borderRadius: 12, padding: spacing.m, zIndex: 10,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 3, elevation: 5,
  },
  ...
  instructionText: {
    ...typography.caption, textAlign: 'center', position: 'absolute', color: 'white',
    backgroundColor: 'rgba(0,0,0,0.7)', padding: spacing.s, borderRadius: 8, zIndex: 4, fontSize: 14,
  },
  ...
  permissionContainer: {
    ...
    backgroundColor: colors.surface, borderRadius: 12, marginVertical: spacing.m,
  },
