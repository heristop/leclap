    ...
    {mode === 'error' ? (
      ...
      <>
        ...
        {mode === 'preview' && previewUrl && (
          <video src={previewUrl} aria-label="Recorded video preview" controls autoPlay loop playsInline className="w-full h-full object-contain bg-black" />
        )}
...
const RecordControls = ({ mode, onStartRecording, onStopRecording }: Pick<ControlsProps, 'mode' | 'onStartRecording' | 'onStopRecording'>) => {
...
export const CameraCapture = ({ onCapture, onClose }: CameraCaptureProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] bg-black/95 backdrop-blur-sm flex flex-col fade-in safe-b">
      <CameraTopBar mode={camera.mode} elapsed={camera.elapsed} onCancel={camera.cancel} onSwitchCamera={camera.switchCamera} />
