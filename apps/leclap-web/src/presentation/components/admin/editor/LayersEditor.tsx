  ...
  return (
    <div className="sm:col-span-2">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_8rem]">
        ...
        <LayerStackPreview layers={list} />
      </div>
    </div>
  );
};

// Live stacked-div swatch: each layer painted in order, base first, extras inset by
// their % geometry — a faithful preview of the composited background.
const LayerStackPreview = ({ layers }: { layers: BackgroundLayer[] }) => (
  <div className="relative aspect-video w-full overflow-hidden rounded-lg border border-foreground/10 sm:sticky sm:top-2 sm:self-start">
    {layers.map((layer, index) => (
      <div key={index} aria-hidden style={cssLayerBackground(layer, index === 0)} />
    ))}
  </div>
);
