...
// full-width grab bar with a grip; `md:hidden` so it only exists in the stacked mobile layout. Drag
// vertically (the ::before overlay pads the touch target to ~44px) or focus it and use ↑/↓ to
// grow/shrink the preview vs the controls.
export const MobileResizeHandle = ({ onResize, onNudge, label }: MobileResizeHandleProps) => {
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className="relative order-2 flex shrink-0 touch-none cursor-row-resize items-center justify-center border-y border-foreground/10 bg-surface-2/40 py-2.5 transition-colors before:absolute before:inset-x-0 before:-inset-y-2 before:content-[''] hover:bg-surface-2/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-500/50 active:bg-surface-2 md:hidden"
    >
