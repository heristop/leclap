...
function openDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      reject(new Error(request.error?.message ?? 'media db open failed'));
    };
  });
}
  ...
  return openDb().then(
    (db) =>
      new Promise<T>((resolve, reject) => {
        ...
        request.onerror = () => {
          reject(new Error(request.error?.message ?? 'media db op failed'));
        };
      })
  );
}
...
const indexedDbBackend: MediaBackend = {
  ...
  get: async (key) =>
    (await tx<MediaRecord | undefined>('readonly', (s) => s.get(key) as IDBRequest<MediaRecord | undefined>)) ?? null,
  delete: async (key) => {
...
describe('templateEditorModel media', () => {
  ...
  it('builds a media:// sentinel for an uploaded track', () => {
    const d = buildDescriptor(
      baseState({ musicEnabled: true, music: { source: 'upload', key: 'k1', label: 'a.mp3' } })
    );
  ...
  it('omits global.music when music is disabled', () => {
    const d = buildDescriptor(
      baseState({ musicEnabled: false, music: { source: 'upload', key: 'k1', label: 'a.mp3' } })
    );
  ...
  it('builds an image_background section for a library background', () => {
    ...
    expect(d.sections?.[0]).toMatchObject({
      ...
      options: { duration: 4, pictureUrl: bg.url ?? '' },
    });
  });
...
describe('materializeTemplateMedia', () => {
  ...
  it('writes an uploaded image and rewrites pictureUrl to an engine path', async () => {
    ...
    await materializeTemplateMedia(
      descriptor,
      sourceWith({ imgK: { bytes: new Uint8Array([2]), ext: 'png' } }),
      target
    );
...
const LibraryCard = ({
  ...
  >
    {kind === 'picture' && (
      <img src={item.url} alt="" loading="lazy" className="mb-2 h-16 w-full rounded object-cover" />
    )}
    ...
    <span className="mt-0.5 block truncate text-[0.65rem] text-gray-400">
      {item.author} · {item.license}
    </span>
    {kind === 'music' && (
      <audio src={item.url} controls preload="none" aria-label={`Preview ${item.title}`} className="mt-2 h-7 w-full" />
    )}
  </button>
);
...
const UploadPane = ({ kind, value, onChange }: MediaPickerProps) => {
  ...
  const onDrop = (files: File[]) => {
    ...
    browserMediaService
      ...
      .catch(() => {
        /* surfaced by the empty selection state */
      });
  };
  ...
  return (
    <div className="grid gap-3 pl-7">
      ...
      <MediaPicker
        ...
        onChange={(background: MediaChoice | null) => {
          onChange({ background });
        }}
      />
    </div>
