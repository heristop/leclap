...
| `@leclap/creative-kit/fonts`                     | `FONTS` — the bundled font set (BebasNeue, Oswald, …) the `drawtext` filter relies on.   |
...
`scripts/copy-core-assets.mjs` (run on dev/build) stages `src/library/*` into each app's static dir, so the web/Expo apps serve the same media the engine renders with — the generated copies are git-ignored.
