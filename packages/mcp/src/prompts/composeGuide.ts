...
function buildText(args: GuideArgs): string {
  ...
  return [
    ...
    '',
    'Workflow:',
    '1. Call get_template_schema for the authoritative shape, and list_templates / get_template to copy a',
    `   polished starting point (intro, quote, titles, reel-portrait, quote-portrait, landscape-spotlight,`,
    '   portrait-spotlight) — they already look professional.',
    '2. Edit the descriptor for the goal. Keep it premium: lean on typography, color grading, and timing.',
    ...
    '',
    'Optional Remotion-assisted authoring: if you think in Remotion (Composition/Sequence) terms, call',
    'get_remotion_authoring_guide, then draft_template_from_remotion_storyboard to turn a storyboard JSON',
    'into a strict descriptor (then validate_template as usual). Remotion is only an authoring mental model —',
    'Do not ask compose_video to render with Remotion; the LeClap engine renders the descriptor on-device.',
    '',
